...
{%- macro sqlserver__end_of_all_times() %}
...
{%- if global_var is mapping -%}
    {%- if 'sqlserver' in global_var.keys()|map('lower') -%}
        {% set end_of_all_times = global_var['sqlserver'] %}
    {%- else -%}
        {%- if execute -%}
            {%- do exceptions.warn("Warning: You have set the global variable 'datavault4dbt.end_of_all_times' to a dictionary, but have not included the adapter you use (sqlserver) as a key. Applying the default value.") -%}
        {% endif %}
        {%- set end_of_all_times = "9999-12-31T23:59:59" -%}
    {% endif %}
{%- elif global_var is not mapping and datavault4dbt.is_something(global_var) -%}
    {%- set end_of_all_times = global_var -%}
{%- else -%}
